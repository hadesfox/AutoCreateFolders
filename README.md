...
这是一个功能强大的Python脚本，可以帮助您在指定目录中自动按顺序编号创建文件夹，并支持在不重启脚本的情况下切换创建目录。
...
- 🔄 **动态目录切换**：支持在脚本运行中随时切换创建目录，无需重启
- 🛡️ **错误检查**：包含完整的错误处理和用户友好的提示信息
- 💡 **智能识别**：自动识别已有文件夹的编号，避免重复
- 📋 **目录验证**：切换目录时验证目录存在性和写入权限
...
```
请输入要创建的文件夹名称和数量（如：test 5，输入'cd'切换目录，'quit'退出）: test
...
```
请输入要创建的文件夹名称和数量（如：test 5，输入'cd'切换目录，'quit'退出）: test 5
正在创建 5 个文件夹...
...
```
请输入要创建的文件夹名称和数量（如：test 5，输入'cd'切换目录，'quit'退出）: cd

=== 切换创建目录 ===
当前目录：D:\workspace\VSC\AutoCreateFolders
请输入新的创建目录（直接回车保持当前目录）: D:\new\folder\path
✅ 已切换创建目录至：D:\new\folder\path
========================================
```
...
```
请输入要创建的文件夹名称和数量（如：test 5，输入'cd'切换目录，'quit'退出）: project
已创建文件夹: D:\new\folder\path\project_01
```
...
|------|------|------|
| `文件夹名称` | 创建单个文件夹 | `test` |
| `文件夹名称 数量` | 批量创建多个文件夹 | `test 5` |
| `changedir` 或 `cd` | 切换创建目录 | `cd` |
| `quit` 或 `exit` | 退出脚本 | `quit` |
...
5. 切换目录时可以直接回车保持当前目录不变
6. 切换目录后会验证新目录的存在性和写入权限
